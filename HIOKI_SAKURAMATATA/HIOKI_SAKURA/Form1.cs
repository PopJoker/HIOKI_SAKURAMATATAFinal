...
namespace HIOKI_SAKURA
{
    public partial class Form1 : Form
    {
        ...
        private void btnStartScan_Click(object sender, EventArgs e)
        {
            ...
            {
                int ch = int.Parse(comboBoxChannel.SelectedItem.ToString());
                slots = new int[] { slot };
                channels = new int[] { ch };
            }
            else
                ...
                try
                {
                    ...
                    while (!token.IsCancellationRequested)
                    {
                        foreach (int ch in channels)
                        {
                            ...
                            string curr = bt3563a?.MeasureCurrent() ?? "N/A"; // 可同時量 BT3563A
                            string line = $"Slot {slot} Ch {ch}: Voltage={volt.Trim()} V, Current={curr.Trim()} A\r\n";
                            AppendTextSafe(textBoxScanResult, line);
                            scanData.AppendLine(line);
                            Thread.Sleep(500);
                        }
                    }
        ...
        private void btnExportCSV_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("目前沒有掃描資料可匯出");
                return;
            }

            SaveFileDialog sfd = new SaveFileDialog();
            sfd.Filter = "CSV files (*.csv)|*.csv";
            sfd.FileName = "ScanResult.csv";

            if (sfd.ShowDialog() == DialogResult.OK)
            {
                File.WriteAllText(sfd.FileName, scanData.ToString());
                MessageBox.Show($"資料已匯出至 {sfd.FileName}");
            }
        }
